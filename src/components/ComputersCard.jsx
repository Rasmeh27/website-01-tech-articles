...
import { ExternalLink, Star, Zap, Award, TrendingUp, Cpu, HardDrive, MemoryStick } from "lucide-react";

const ComputersCard = ({ computer, index }) => {
    ...
    const getComponentIcon = (title) => {
        if (title.toLowerCase().includes('procesador') || title.toLowerCase().includes('ryzen') || title.toLowerCase().includes('intel')) {
            return <Cpu className="h-4 w-4" />;
        } else if (title.toLowerCase().includes('ssd') || title.toLowerCase().includes('nvme')) {
            return <HardDrive className="h-4 w-4" />;
        } else if (title.toLowerCase().includes('ddr') || title.toLowerCase().includes('memoria')) {
            ...
            <div className="p-6">
                ...
                <div className="flex flex-wrap gap-2 mb-6">
                    <div className="flex items-center bg-blue-50 text-blue-700 px-3 py-1 rounded-full text-xs font-medium">
                        {getComponentIcon(computer.title)}
                        <span className="ml-1">Alto Rendimiento</span>
                    </div>
                    {isTopRated && (
